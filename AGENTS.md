...
方法试下来不太行：必须改方法（simplify / delete / change）再测。内循环可以一直下去。禁止：原方法再跑一轮同类 EXP。不要因为轮次多就停循环。Support、retry、凑 seed 不算「已经试过」。指标没涨但方法改了并进入下一判别，算有成果。
...
W2 设计一个判别实验（默认从很小开始）
 ...
 → W3 机制诊断（exit 0 ≠ 科学成功）
 → W4 问：对方法意味着什么？不行就改方法，回 W2 再测
```

外循环（低频）：只有 Core Idea / 路线要重构、下一科学问题不清，或方法不行却点不出改什么，才 `W4 → W1`。**不能**借 W1 换掉 PROJECT Research Goal。内循环本身可以一直下去。
...
| 新会话且 Next 已点名普通 EXP | 直接继续内循环 | 下表 W2–W3 | `workspace-resume`、`research-loop`（**除非** Next 是失败方法的同类再跑：先改方法再 W2） |
...
| `W4` 点不出方法改什么 / Next 不清 / Level 2 | 换挡，不换 Goal | `research-loop` | 把 Goal 改成另一个课题 |
...
| `research-loop` | W1；W4 且下一科学问题不清；新证据使已点名 Next 失效；方法不行却点不出改什么 | 内循环已清楚且前提仍成立（W2 有 EXP / running / W3 有产物；失败后已改方法再测） | **只调度**，自己不跑实验、不解读 |
| `experiment-design` | 需要新的判别问题；测刚改过的方法 | 只是修 parser；只是为凑次数再跑 seed；失败方法的同类再跑 | 登记 EXP；默认从小开始；方法不行就登记测新方法的 EXP，不要停循环 |
