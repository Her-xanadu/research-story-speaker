...
若上一**科学** EXP 表明当前方法不行，而本任务仍是同一 Question、同一方法、同一对照，则不要新 EXP-ID。先改方法，再登记**测改过的方法**的 EXP（仍内循环）。隔离失败后登记「新加模块」的 EXP，须已有针对这次失败条件的文献 successor 或明确 closest contrast；不要把现场发明的模块写成新 EXP-ID。点不出删除/隔离、也写不出该查什么失败条件，才是换问题式停滞（`story-loop.md` §停滞处理）。Support / retry / 为凑 seed 的重复仍走 §Support-task rule。一次 exploratory 负结果若下一轮已是删除或新隔离，可以登记。
