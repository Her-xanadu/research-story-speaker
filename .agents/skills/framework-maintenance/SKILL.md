...
description: >-
  ...
  Story science. Doctor / upper / experience-to-skill are modes, not extra Skills.
---
...
records — use scientific Skills instead. Do **not** create a 16th Skill for
...
| 10 | Skill count is **15** | Original 10 plus `idea-evaluation`, `evidence-verification`, `workspace-setup`, `framework-extension` (maintainer — **not** a research-loop step), and `monitor-experiment` (W2 wait after launch; **not** a new Stage). New Skills require `framework-extension` ladder review; do not add a 16th without audit. |
...
Also verify: `.agents/skills/` is the **only** canonical Skill root (**15**
Skills, including `idea-evaluation`, `evidence-verification`,
`workspace-setup`, `framework-extension`, and `monitor-experiment`); subagent
...
- Do not add a 16th Skill without `framework-extension` review.
  `framework-extension` is the maintainer/contributor Skill for designing
  integrations; `workspace-setup` is harness personalization;
  `monitor-experiment` is the W2 wait owner after launch (not a new Stage);
  doctor / upper / experience-to-skill are modes of this Skill.
